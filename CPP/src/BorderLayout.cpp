...
void BorderLayout::AddPopUp(QLabel* item) {
    // Create popup widget
    QDialog* popup = new QDialog(this->parentWidget());
    popup->setModal(true);
    popup->setWindowTitle(item->text());
    QLabel* popupLabel = new QLabel(popup);
    popupLabel->setText(item->text());
    popupLabel->setAlignment(Qt::AlignCenter);

    // Set up layout and show popup
